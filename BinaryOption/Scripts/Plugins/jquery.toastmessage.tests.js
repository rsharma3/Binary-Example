(function (c) {
    var b = { inEffect: { opacity: "show" }, inEffectDuration: 600, stayTime: 3000, text: "", sticky: false, type: "notice", position: "top-center", closeText: "", close: null };
    var a = { init: function (d) {
        ...
        rt = c("<p></p>");
        d = c("<div></div>").hide().addClass("toast-item toast-type-" + g.type).appendTo(j).html(c("<p>").append(g.text)).animate(g.inEffect, g.inEffectDuration).wrap(rt).wrap(e);
        ...
        if (navigator.userAgent.match(/MSIE 6/i)) {
            j.css({ top: document.documentElement.scrollTop });
                ...
                setTimeout(function() {
                    c().toastmessage("removeToast", j, g);
                }, g.stayTime);
            }
            return d;
        ...
        e.animate({ opacity: "0" }, 100, function () {
            e.animate({ height: "0px" }, 100, function() {
                e.remove();
            });
        });
        if (d && d.close !== null) {
