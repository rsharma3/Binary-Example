    ...
    var methods = {
        ...
        */
        _validateField: function (field, options, skipAjaxValidation) {
            ...
            var fieldName = field.attr("title");
            var promptText = "";
            var required = false;
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                switch (rules[i]) {
                    ...
                    case "minSize":
                        ...
                        break;
                    case "minSizeYear":
                        errorMsg = methods._minSizeYear(field, rules, i, options);
                        break;
                    case "maxSize":
            ...
            if ((fieldType == "radio" || fieldType == "checkbox") && $("input[title='" + fieldName + "']").size() > 1) {
                field = $($("input[title='" + fieldName + "'][type!=hidden]:first"));
            ...
            if (options.isError) {
                ...
                field.parent().append('<div class="errorMsg">' + promptText + '</div>');
        ...
        */
        _required: function (field, rules, i, options) {
            switch (field.attr("type")) {
                ...
                case "radio":
                case "checkbox":
                    var name = field.attr("title");
                    if ($("input[title='" + name + "']:checked").size() == 0) {

                        if ($("input[title='" + name + "']").size() == 1)
        ...
        */
        _greater: function (field, rules, i, options) {
            var greaterField = rules[i + 1];

            if (field.attr('value') < $("#" + greaterField).attr('value'))
                return options.allrules.greaterRange.alertText;
        },
        /**
        ...
        */
        ...
        _minSizeYear: function (field, rules, i, options) {
            var min = rules[i + 1];
            var len = field.attr('value').length;
        ...
        */
        _past: function (field, rules, i, options) {
            ...
            var p = rules[i + 1];
            var pdate = (p.toLowerCase() == "now") ? new Date() : methods._parseDate($(p).attr('value'));
        ...
        */
        _maxCheckbox: function (field, rules, i, options) {
            ...
            var groupname = field.attr("title");
            var groupSize = $("input[title='" + groupname + "']:checked").size();
        ...
        _calculatePosition: function (field, promptElmt, options) {
            ...
            switch (options.promptPosition) {
                ...
                case "topRight":
                    if (overflow)
                    // Is the form contained in an overflown container?
                        promptleftPosition += fieldWidth - 145;
                    else {
...
$(document).ready(function () {
    ...
    $('input[type=text],input[type=password]').blur(function () {
        ...
        $("#" + Id).parent().find(".errorMsg").remove();

        if ($("#" + Id).validationEngine('validateField', '#' + Id)) {
            $("#" + Id).addClass("validationMessage");

            return false;
        }
        else {
            $("#" + Id).removeClass("validationMessage");
        }
